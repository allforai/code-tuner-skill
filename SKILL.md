...
1. **Phase 0 必须用户确认** — 架构类型判错，后续全错
2. **不自动重构** — 只输出报告和任务清单，由用户决定执行
3. **两种模式贯穿始终** — 每条发现都给出两种模式下的不同建议
4. **宽进严出** — 格式校验在入口层，业务校验在业务层，数据层不越界
5. **简单透传可跳层** — 纯 CRUD 无业务逻辑时，入口层可直接调数据层
6. **不过度抽象** — 同时检测"该抽象没抽象"和"不该抽象却抽象了"
7. **名称不重要，职责才重要** — 通过依赖模式识别层级，不靠目录名
...
项目路径是 /path/to/project。
画像文件在 .tuner/tuner-profile.json。
```

...
1. 将 `tuner-report.md` 保存到 `.tuner/tuner-report.md`
2. 将 `tuner-tasks.json` 保存到 `.tuner/tuner-tasks.json`
3. 如果 `.tuner/` 目录不存在，自动创建
...
详细报告：`.tuner/tuner-report.md`
任务清单：`.tuner/tuner-tasks.json`
```

### 趋势对比展示

若 `.tuner/` 中存在上次的 `tuner-report.md`，在保存新报告前先解析旧报告的分数和问题列表，进行对比。

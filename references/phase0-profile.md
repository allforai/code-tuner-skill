...
Phase 0 的完整输出为 `tuner-profile.json`，保存在项目的 `.tuner/` 目录下。
...
确认通过后，将最终版本写入 `.tuner/tuner-profile.json`，进入后续 Phase。
...
- [ ] 展示确认清单，等待用户确认
- [ ] 用户确认通过后，写入 `.tuner/tuner-profile.json`

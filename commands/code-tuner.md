...
Phase 0 是所有模式的必经阶段。执行前先检查项目中是否已存在 `.tuner/tuner-profile.json`：
...
**三项都必须用户确认后才能进入后续 Phase。**

## 各 Phase 执行要求

每个 Phase 执行前，用 Read 工具加载对应的 reference 文档获取详细规则和检测方法。

每个 Phase 完成后，将结果写入 `.tuner/` 目录下对应的 JSON 文件。
...
> 完整报告已保存至: `.tuner/tuner-report.md`
> 重构任务清单: `.tuner/tuner-tasks.json`
```
